...
app.layout = dbc.Container([
    dbc.Row([
        ...
        dbc.Col([
            ...
            dcc.Graph(id='combined-chart'),  # Add combined chart here
        ], width=9),
    ]),
], fluid=True)
...
@app.callback(
    [Output('output-container-date-picker-range', 'children'),
     ...
     Output('combined-chart', 'figure')],  # Add output for combined chart
...
def update_output(start_date, end_date, flower_patterns, sizes, dims, search_input):
    ...
    combined_data = dff.groupby('时间').agg({'全国现货库存': 'sum', '全国昨日出库商品件数': 'sum'})
    ...
    combined_fig.add_trace(
        go.Scatter(x=combined_data.index, y=combined_data['全国现货库存'], mode='lines+markers', name='全国现货库存'))
    combined_fig.add_trace(
        go.Scatter(x=combined_data.index, y=combined_data['全国昨日出库商品件数'], mode='lines+markers',
                   name='全国昨日出库商品件数'))

    return [f'Selected Start Date: {start_date}, Selected End Date: {end_date}', fig, city_fig, combined_fig]

...
app.layout = html.Div([
    ...
    html.Div([
        html.Label('Flower Patterns: '),
        dcc.Dropdown(
            id='flower-pattern',
            options=[{'label': 'ALL', 'value': 'ALL'}] + [{'label': i, 'value': i} for i in df['花纹'].unique()],
            value=['ALL'],
            multi=True,
        ...
        dcc.Dropdown(
            id='size',
            options=[{'label': 'ALL', 'value': 'ALL'}] + [{'label': i, 'value': i} for i in df['尺寸'].unique()],
            value=['ALL'],
            multi=True,
...
def update_output(start_date, end_date, flower_patterns, sizes, dims, search_input):
    ...
    return f"Start Date: {start_date} End Date: {end_date}", fig

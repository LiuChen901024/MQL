...
def filter_data(start_date, end_date, selected_patterns, selected_sizes, selected_dims, search_value):
...
@app.callback(
    ...
    Output('profit-graph-output', 'figure'),   # 新增了一个输出
...
def update_output(start_date, end_date, selected_patterns, selected_sizes, selected_dims, search_value):
    try:
        filtered_df = filter_data(start_date, end_date, selected_patterns, selected_sizes, selected_dims, search_value)
    ...
    # 计算每个花纹的销售利润
    grouped_df_profit = filtered_df.groupby('花纹', as_index=False).agg({'成交金额': 'sum', '成本': 'sum'})
    grouped_df_profit['销售利润'] = round(grouped_df_profit['成交金额'] - grouped_df_profit['成本'])
    # 创建销售利润的柱状图
    ...
    # 返回图像
    return fig, fig_combo, fig_combo_2, fig_profit  # 新增了一个返回值

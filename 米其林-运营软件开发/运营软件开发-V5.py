...
def update_output(start_date, end_date, selected_patterns, selected_sizes, selected_dims, search_value):
    ...
    fig_combo = go.Figure(data=[
        ...
        go.Scatter(name='成交转化率', x=grouped_df_by_date["时间"], y=grouped_df_by_date["转化率"], yaxis='y2',
                   mode='lines'),
        go.Scatter(name='转化率标签', x=grouped_df_by_date["时间"], y=grouped_df_by_date["转化率"], yaxis='y2',
                   mode='text', text=[f"{i:.2%}" for i in grouped_df_by_date["转化率"]], textposition='top center')
    ])
